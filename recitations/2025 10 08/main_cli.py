...
from ai_ppp_python_review_solutions import TodoList # remove this when using code in todo_list.py
...
def main_menu():
    ...
    while True:
        ...
        if choice == 1: # Select a list
            ...
            list_choice = get_integer_input(f"Enter the list number to select (1-{num_lists}): ", 1, num_lists)
            ...
            file_path = input(f"Enter the path/filename to load (default: {DEFAULT_FILE_PATH}): ") or DEFAULT_FILE_PATH
            if not os.path.exists(file_path):
                print(f"Error: File '{file_path}' not found.")
                continue
                
            list_name = input(f"Enter a name for the loaded list (default: {os.path.basename(file_path).split('.')[0]}): ")
            if not list_name:
                list_name = os.path.basename(file_path).split('.')[0]
            
            if list_name in loaded_lists:
                print(f"Error: List with name '{list_name}' already loaded. Remove it first.")
                continue
            ...
            remove_choice = get_integer_input(f"Enter the list number to remove from memory (1-{num_lists}): ", 1, num_lists)
